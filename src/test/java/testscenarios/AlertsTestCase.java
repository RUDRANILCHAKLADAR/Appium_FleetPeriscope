...
public class AlertsTestCase extends BaseTest {
    ...
    protected void deInit() {
        TestUtils.logOutUser(alertsPage,alertPageDriver);
    }

    public void  alertsPageSetup() {
        TestUtils.logInUser(alertsPage, alertPageDriver, "Fleet360A", "Password@1",this);
        TestUtils.waitForVisibility(alertsPage.alertsBottomBar,alertPageDriver);
        alertsPage.alertsBottomBar.click();
        TestUtils.handlelocationPopup(this,alertsPage,alertPageDriver);
        new WebDriverWait(alertPageDriver, Duration.ofSeconds(60)).until(ExpectedConditions.visibilityOf(alertsPage.getAlertLists()));
        Assert.assertTrue(alertsPage.getAlertLists().isDisplayed(),"The Alert List is not displayed ");
    }


    @Test(priority = 0)
    public void testAlertsScreenUIElements()
    {
        Assert.assertEquals(alertsPage.getAlertTitleText(),alertsPage.alertTitle,"The Alerts - 7 Day History title text is not displayed");
        Assert.assertTrue(alertsPage.getFilterIcon().isEnabled(),"Filter icon is not enabled on alerts screen");
       // Assert.assertTrue(alertsPage.getTotalItemsCount().isDisplayed(),"The header count is not displayed");
